        ...
        <transition  name="slide-fade">
          <div v-show="open" class="z-50 origin-top-right absolute right-0 mt-4 w-56 rounded-md shadow-lg">
            <div class="rounded-md bg-white shadow-xs">
              <div class="py-1">
                <a :href="`/patients/${id}`" class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900">Account Details</a>
...
import ChangePatientPassword from '../extras/modals/ChangePatientPassword'
  export default {
    ...
    components:{Icon,ChangePatientPassword},
    data(){
      return{
        open:false,
        imgError : true
    ...
    methods:{
      ...
      changePassword(){
        fire.$emit('changepatientpassword', {id:this.id,name:this.name })
      },
      deletePatient(){
         this.$snotify.warning('This patient will be deleted', 'Warning', {
          timeout: 5000,
