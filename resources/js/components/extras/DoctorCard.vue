    ...
    <div class="bg-white rounded-lg px-4 py-6 relative">
        ...
        <transition  name="slide-fade">
          <div v-show="open" class="z-50 origin-top-right absolute right-0 mt-4 w-56 rounded-md shadow-lg">
            <div class="rounded-md bg-white shadow-xs">
              <div class="py-1">
                <a :href="`/doctors/${id}`" class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900">Account Details</a>
                ...
                <button @click.prevent="deleteDoctor" type="submit" class="block w-full text-left px-4 py-2 text-sm leading-5 text-red-700 hover:bg-red-100 hover:text-red-900 focus:outline-none focus:bg-red-100 focus:text-red-900">
                   Delete Doctor
                </button>
              </div>
            </div>
      ...
      <div class="flex justify-center items-center flex-col">
        ...
        <div class="text-center mt-4">
          <p class="text-lg font-bold text-gray-600">Dr. {{ name }}</p>
          <div>
            <form>
              <div class="rating mx-auto">
                <input type="radio" v-model="avgrating" value="5" /><label for="star5"></label>
                <input type="radio" v-model="avgrating" value="4" /><label for="star4"></label>
                <input type="radio" v-model="avgrating" value="3" /><label for="star3"></label>
                <input type="radio" v-model="avgrating" value="2" /><label for="star2"></label>
                <input type="radio" v-model="avgrating" value="1" /><label for="star1"></label>
              </div>
            </form>
          </div>

          <p class="text-sm text-gray-500">{{ username }}</p>
          <span class="inline-flex items-center px-3 py-0.5 rounded-full text-xs mt-2 font-medium leading-5 bg-green-100 text-green-800">
            Standard
          </span>
        </div>
      </div>
...
import { mixin as clickaway } from 'vue-clickaway';
  export default {
    ...
    methods:{
      changePassword(){
        fire.$emit('changedoctorpassword', {id:this.id,name:this.name })
      },
      away(){
        this.open = false
      },
      deleteDoctor(){
         this.$snotify.warning('This doctor will be deleted', 'Warning', {
          timeout: 5000,
          ...
          buttons: [
            ...
            
            {
              ...
              .then(resp=>{
                fire.$emit('doctorDeleted')
                this.$snotify.success('Doctor deleted successfully', "Success");
              }).catch(error=>{
                console.log(error)
              })
    ...
    mounted(){
      let myArray = this.rating.map(a =>  a.rating)
      this.avgrating = Math.round(this.sum(myArray)/myArray.length)
    }
  }
</script>
